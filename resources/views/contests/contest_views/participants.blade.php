...
<table class="table table-bordered table-hover text-center">
    ...
    @foreach($participants as $participant)
        ...
        <tr>
            {{--Username--}}
            <td>
                <a class="testing-participant-username"
                   href="{{ route(\App\Utilities\Constants::ROUTES_PROFILE, $userName) }}">
                    {{ $userName }}
                </a>
            </td>
            ...
            @if($isOwnerOrOrganizer)
                ...
                <td class="text-center">
                    @include('components.action_form', [
                        'url' => route(\App\Utilities\Constants::ROUTES_CONTESTS_PARTICIPANTS_DELETE, [$contestID, $userName]),
                        'method' => 'DELETE',
                        'confirm' => true,
                        'confirmMsg' => "'Are you sure want to remove this participant? This action cannot be undone!'",
                        'btnIDs' => "",
                        'btnClasses' => 'btn btn-link text-dark',
                        'btnTxt' => 'Remove'
                    ])
                </td>
            @endif
